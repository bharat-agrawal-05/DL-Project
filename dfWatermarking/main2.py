...
class Config:
    def __init__(self, 
                 ...
                 inference_steps: int = 100, # Keep it at 100 for this example
                 embedding_step: int = 80, # At which denoising step to embed
                 save_intermediate_every_n_steps: int = 20 # Save more frequently to see effect
                ):
        self.SEED = 42
        self.prompt = prompt
        ...
        self.inference_steps = inference_steps     # <<<<<<< CHANGED
        self.guidance_scale = 7.5

        # Intermediate saving
        self.save_intermediate_every_n_steps = save_intermediate_every_n_steps # Save more frequently to see effect

        # Technical
        self.mixed_precision = "fp16"
        self.gradient_accumulation_steps = 1

        # Message Embedding Parameters <<<<<<< NEW SECTION
        self.binary_message = binary_message
        self.embedding_step = embedding_step # At which denoising step to embed
        self.embedding_strength = 0.2 # How much to modify pixel values (0 to 1)
        self.embedding_patch_size = 4 # Size of the patch for each bit (e.g., 4x4 pixels)
...
# ... (import_model_class_from_model_name_or_path, load_models_and_tokenizer, setup_accelerator_and_logging remain largely the same)
...
# _encode_prompt remains the same
...
def embed_message_in_image_tensor(image_tensor: torch.Tensor, binary_message: str, 
    ...
    """
    Embeds a binary message into an image tensor by modifying patches.
    image_tensor: (B, C, H, W), expected to be in [0, 1] range.
    binary_message: String of '0's and '1's.
    patch_size: Size of the square patch for each bit.
    strength: How much to modify pixel values (e.g., 0.1 for 10% change).
    start_x, start_y: Top-left coordinates to start embedding patches.
    Returns modified image_tensor.
    """
    if image_tensor.dim() != 4:
        raise ValueError("Image tensor must be 4D (B, C, H, W)")
    if image_tensor.shape[0] > 1:
        logger.warning("Message embedding is applied to the first image in the batch only for this demo.")

    img_c, img_h, img_w = image_tensor.shape[1], image_tensor.shape[2], image_tensor.shape[3]
    modified_image = image_tensor.clone() # Work on a copy

    current_x, current_y = start_x, start_y

    for bit_idx, bit in enumerate(binary_message):
        if current_y + patch_size > img_h: # Move to next column if out of height
            current_y = start_y
            current_x += patch_size
        
        if current_x + patch_size > img_w:
            logger.warning(f"Ran out of space to embed full message at bit {bit_idx}. Message truncated.")
            break

        patch = modified_image[0, :, current_y : current_y + patch_size, current_x : current_x + patch_size]

        if bit == '1':
            # Example: make patch brighter by 'strength'
            patch += strength
        elif bit == '0':
            # Example: make patch darker by 'strength'
            patch -= strength
        
        modified_image[0, :, current_y : current_y + patch_size, current_x : current_x + patch_size] = patch.clamp(0, 1)
        
        current_y += patch_size # Move to the next patch position downwards
...
def generate_images(
    ...
    # Convert binary message string to a list of integers (0s and 1s)
    # binary_message_bits = [int(b) for b in config.binary_message]
    ...
    for batch_idx in range(num_unet_batches):
        ...
        # Store initial latents if we need to re-noise for message injection
        initial_latents_for_batch = torch.randn(
            latents_shape, generator=generator, device=accelerator.device, dtype=weight_dtype
        )
        latents = initial_latents_for_batch * noise_scheduler.init_noise_sigma # Apply initial noise sigma
        ...
        # Shape checks (already present and good)
        ...
        for i, t in enumerate(tqdm(timesteps, desc=f"Batch {batch_idx+1} Denoising")):
            current_denoising_step = config.inference_steps - i # Step number from 100 down to 1
            ...
            if do_classifier_free_guidance:
                ...
                noise_pred = noise_pred_out # If not CFG, unet output is directly the noise_pred
            ...
            if current_denoising_step == config.embedding_step:
                ...
                with torch.no_grad():
                    # 1. Predict x0 (clean image) from current latents (xt) and noise_pred (epsilon_t)
                    alpha_prod_t = noise_scheduler.alphas_cumprod[t]
                    beta_prod_t = 1 - alpha_prod_t
                    pred_original_sample = (latents - beta_prod_t**(0.5) * noise_pred) / alpha_prod_t**(0.5)
                    
                    # 2. Decode x0 latents to image space
                    # Ensure pred_original_sample is on the correct device and dtype for VAE
                    ...
                    # 3. Normalize image to [0, 1] for embedding
                    image_for_embedding = (decoded_x0_image / 2 + 0.5).clamp(0, 1)
                    
                    # Save pre-embedding image for comparison
                    if accelerator.is_main_process:
                        ...
                        torchvision.utils.save_image(image_for_embedding[0].float().cpu(), pre_embed_path) # Save first image in batch
                    ...
                    modified_image_for_embedding = embed_message_in_image_tensor(
                     ...
                     # Save post-embedding image for comparison
                    if accelerator.is_main_process:
                    ...
                    # 5. Convert modified image back to [-1, 1] and then to latents
                    modified_image_scaled_back = (modified_image_for_embedding * 2 - 1).to(sd_vae.device, dtype=sd_vae.dtype)
                    # Encode expects (B, C, H, W) on correct device and dtype
                    modified_latents_dist = sd_vae.encode(modified_image_scaled_back).latent_dist
                    modified_x0_latents = modified_latents_dist.sample() # Get mean or sample
                    modified_x0_latents = modified_x0_latents * config.sd_vae_scale_factor # Scale back

                    # 6. Re-noise the modified x0_latents to the current timestep t
                    # We need the original noise that would have led from initial_latents_for_batch to this xt if no message was embedded
                    # However, a simpler way is to add noise to our new x0 based on current timestep t.
                    # Get noise according to scheduler for timestep t.
                    # Note: initial_latents_for_batch was pure Gaussian noise.
                    # The noise for add_noise is also pure Gaussian.
                    noise_for_current_step = torch.randn_like(modified_x0_latents)
                    
                    # Replace current latents with the re-noised modified latents
                    # Make sure modified_x0_latents and noise_for_current_step are on the same device as latents
                    latents = noise_scheduler.add_noise(
                        ...
                        t.unsqueeze(0) # timesteps arg expects a tensor
                    ).to(latents.dtype)
                    logger.info(f"    Message embedded. New latents shape: {latents.shape}")
            # <<<<<<<<<<<<<<<<<<<<<<<< END MESSAGE EMBEDDING LOGIC >>>>>>>>>>>>>>>>>>>>>>>>>>
...
if __name__ == "__main__":
    # Example usage:
    # Default 32-bit message
    # config_run = Config() 
    
    # Custom message
    # config_run = Config(binary_message="11110000111100001111000011110000")
    
    # Shorter run for testing
    config_run = Config(
        num_images_per_prompt=1, 
        inference_steps=100, # Keep it at 100
        embedding_step=80,   # Embed at step 80 (counting down from 100)
        save_intermediate_every_n_steps=10 # Save every 10 steps
    )
    
    # To test without embedding, you could set embedding_step to 0 or > inference_steps
    # config_run.embedding_step = 0 

    main(config_run)

# DESCRIPTION OF EMBEDDING FUNCTION: Directly adding the binary message to the image tensor.