...
def generate_images_WIP(accelerator, class_index, single_class_text_embeddings, uncond_embeddings,
                      ...
                      videoseal_model_instance, target_binary_message_tensor, # target is now [B, 128]
    ...
    for timestep_idx, timesteps in enumerate(tqdm(timesteps_tensor[:-1], desc=f"Class {class_index} Steps")):
        ...
        if apply_dgm_guidance:
            ...
            with torch.enable_grad():
                ...
                raw_detector_output = videoseal_model_instance.detector(imgs_res_for_detector) # Output: [B, 1+N_MODEL_BITS, H, W] or [B, 1+N_MODEL_BITS]
                
                if raw_detector_output.ndim == 4:
                    detected_logits_pooled = torch.mean(raw_detector_output, dim=[2,3]) # [B, 1+N_MODEL_BITS]
                elif raw_detector_output.ndim == 2:
                    detected_logits_pooled = raw_detector_output # [B, 1+N_MODEL_BITS]
                else:
                    raise ValueError(f"Unexpected detector output shape: {raw_detector_output.shape}")
            
            # detected_logits_pooled is [B, 1+N_MODEL_BITS]
            # message_logits should be [B, N_MODEL_BITS]
            message_logits = detected_logits_pooled[:, 1:] 
            
            # **IMPORTANT**: If N_MODEL_BITS (e.g. 256) from videoseal_model_instance
            # does not match config.num_watermark_bits (now 128), this will error out
            # in BCE loss. We assume they match due to loading the correct model.
            if message_logits.shape[1] != config.num_watermark_bits:
                # This is a fallback if you load a 256-bit model but target 128 bits.
                # It's not ideal as you are only guiding on a subset of what the model sees.
                print(f"Warning: VideoSeal model produced {message_logits.shape[1]} logit bits, "
                      f"but target is {config.num_watermark_bits} bits. Slicing model output.")
                message_logits = message_logits[:, :config.num_watermark_bits]


            current_batch_size = message_logits.shape[0]
            expanded_target_message = target_binary_message_tensor.expand(current_batch_size, -1) # Target is [B, 128]
            
            loss_watermark_val = F.binary_cross_entropy_with_logits(message_logits, expanded_target_message.float(), reduction="sum")
            ...
            current_step_bit_acc = calculate_bit_accuracy(message_logits.detach(), expanded_target_message)
        ...
        if config.save_intermediate_images and (timestep_idx + 1) % config.save_intermediate_every_n_steps == 0 :
            with torch.no_grad():
                ...
                save_tag = "dgm_step" if apply_dgm_guidance else "cfg_only"
                grad_guided_exists_this_iter = apply_dgm_guidance and ('grad_guided' in locals()) 
                if grad_guided_exists_this_iter and grad_guided is None : save_tag += "_nograd"
    ...
    with torch.no_grad():
        ...
        if videoseal_model_instance is not None and config.watermark_guidance_weight > 0:
            ...
            final_message_logits = final_detected_logits_pooled[:, 1:]
            if final_message_logits.shape[1] != config.num_watermark_bits:
                print(f"Warning (Final Eval): VideoSeal model produced {final_message_logits.shape[1]} logit bits, "
                      f"but target is {config.num_watermark_bits} bits. Slicing model output for accuracy.")
                final_message_logits = final_message_logits[:, :config.num_watermark_bits]

            current_final_batch_size = final_message_logits.shape[0]
            expanded_target_final = target_binary_message_tensor.expand(current_final_batch_size, -1)
            final_bit_accuracy = calculate_bit_accuracy(final_message_logits, expanded_target_final)
...
class Config:
    def __init__(self):
        self.SEED = 5000
        self.name = "synthetic_cifar_videoseal_128bit_target"  # New name for clarity
        self.save_syn_data_path = "./synthetic_cifar_videoseal_128bit_target" # New path
        self.checkpoints_dir = "./checkpoints"
        self.generate_nums = 1  # Generate one "set"
        self.num_images_to_generate_total = 5 # Target 5 images in total
        self.batch_size_generation = 1
        self.STABLE_DIFFUSION = "runwayml/stable-diffusion-v1-5"; self.SD_REVISION = None
        self.inference_nums = 20 
        self.guided_scale = 7.5 
        self.lr_dgm = 0.02 
        self.watermark_guidance_weight = 1.0 
        self.skip_dgm_initial_ratio = 0.2 
        self.grad_clip = 0.5 
        self.sd_vae_scale_factor = 0.18215
        
        # Use the default videoseal checkpoint name, which loads the available (likely 256-bit) model
        self.videoseal_ckpt_path = 'videoseal' 
        
        # Set your target message to 128 bits
...
def main(config:Config):
    ...
    try:
        ...
        # Check model's bit capacity if possible (not a standard attribute)
        # model_bits = getattr(videoseal_model_instance.detector, 'nbits', None) # Assuming detector has nbits
        # if model_bits is not None and model_bits != config.num_watermark_bits:
        # print(f"Warning: Loaded VideoSeal model is for {model_bits} bits, but config targets {config.num_watermark_bits} bits.")
        ...
        print("Proceeding without VideoSeal guidance or using a dummy model if that was intended.")
    ...
    if videoseal_model_instance and len(config.target_watermark_message_str) != config.num_watermark_bits:
        # This check is good, but the core issue is the loaded model's capacity vs target.
        raise ValueError(f"Config message length ({len(config.target_watermark_message_str)}) "
                         f"mismatches config.num_watermark_bits ({config.num_watermark_bits}).")
