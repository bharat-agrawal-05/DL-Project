...
def generate_images_WIP(accelerator, class_index, single_class_text_embeddings, uncond_embeddings,
    ...
    timesteps_tensor = noise_scheduler.timesteps.to(accelerator.device) # Total timesteps (e.g., 20)
                                                                      # Loop is for timesteps_tensor[:-1] (19 steps for N=20)
                                                                      # timestep_idx goes from 0 to N-2
    ...
    # Calculate start and end DGM application indices based on total actual steps in the loop
    num_loop_steps = len(timesteps_tensor[:-1]) # This is config.inference_nums - 1
    ...
    for timestep_idx, timesteps in enumerate(tqdm(timesteps_tensor[:-1], desc=f"Class {class_index} Steps")):
        ...
        apply_dgm_guidance = (
            config.watermark_guidance_weight != 0 and
            timestep_idx >= dgm_start_step_idx and  # Apply after initial skip
            timestep_idx < dgm_end_step_idx        # Do not apply in the final skip window
        )
        ...
        if config.save_intermediate_images and (timestep_idx + 1) % config.save_intermediate_every_n_steps == 0 :
            with torch.no_grad():
                ...
                # Check if grad_guided was computed in this iteration if DGM was applied
                grad_guided_computed_this_iter = apply_dgm_guidance and ('grad_guided' in locals() and grad_guided is not None)
                if apply_dgm_guidance and not grad_guided_computed_this_iter : # If DGM applied but grad was None
                    save_tag += "_nograd"
...
class Config:
    def __init__(self):
        self.SEED = 5000
        self.name = "synthetic_cifar_videoseal_v8_dgm_window" 
        self.save_syn_data_path = "./synthetic_cifar_videoseal_v8_dgm_window"
        ...
        self.lr_dgm = 0.3 
        self.watermark_guidance_weight = 1.5 
        self.skip_dgm_initial_ratio = 0 # Skip DGM for first 20% of steps
        self.skip_dgm_final_ratio = 0.05   # ADDED: Skip DGM for last 20% of steps
        self.grad_clip = 15 
        self.sd_vae_scale_factor = 0.18215
        
        self.videoseal_ckpt_path = 'videoseal' # Loads default 256-bit model
        
        self.target_watermark_message_str = "10" * 64; self.num_watermark_bits = 128 # Target 128 bits
        
        self.label_name = True; self.data_type = "cifar10"
        self.save_intermediate_images = True 
        self.save_intermediate_every_n_steps = 10 # Save less frequently if many steps
