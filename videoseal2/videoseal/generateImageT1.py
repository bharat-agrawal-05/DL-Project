...
def generate_images_WIP(accelerator, class_index, single_class_text_embeddings, uncond_embeddings,
    ...
    for timestep_idx, timesteps in enumerate(tqdm(timesteps_tensor[:-1], desc=f"Class {class_index} Steps")):
        ...
        if apply_dgm_guidance:
            ...
            detected_logits = detected_logits_pooled
            message_logits = detected_logits[:, 1:]
            ...
            # Calculate loss for DGM guidance
            ...
            # Calculate bit accuracy for this DGM step's predicted x0
            # We need message_logits *before* gradients are computed through it for the loss.
            # So, we use the message_logits derived from the current x0 prediction.
            # Since message_logits is already calculated, we can use it directly.
            # No need to detach for accuracy calculation.
            current_step_bit_acc = calculate_bit_accuracy(message_logits.detach(), expanded_target_message) # Detach logits for acc calc only
...
class Config:
    def __init__(self):
        self.SEED = 5000
        self.name = "synthetic_cifar_videoseal_v6_detailed_log" 
        self.save_syn_data_path = "./synthetic_cifar_videoseal_v6_detailed_log"
        ...
        self.inference_nums = 100 # Increased for more DGM steps to observe
        self.guided_scale = 7.5 # Common default
        self.lr_dgm = 0.3 # Might need tuning
        self.watermark_guidance_weight = 2.5 # Might need tuning
        self.skip_dgm_initial_ratio = 0.2 # Skip DGM for first 20% of steps
        self.grad_clip = 15 # Tunable
        self.sd_vae_scale_factor = 0.18215
        self.videoseal_ckpt_path = 'videoseal'
        self.target_watermark_message_str = "11" * 128; self.num_watermark_bits = 256
        self.label_name = True; self.data_type = "cifar10"
        self.save_intermediate_images = True # Control whether to save intermediate steps
        self.save_intermediate_every_n_steps = 5 # If saving, how often
        # self.debug_print_freq = 1 # Not needed if logging every DGM step
...
def main(config:Config):
    ...
    try:
        ...
        print(f"Error loading VideoSeal model: {e}"); config.watermark_guidance_weight = 0
    ...
    print(f"Target message: shape {target_binary_message_tensor.shape}, dtype {target_binary_message_tensor.dtype}")
