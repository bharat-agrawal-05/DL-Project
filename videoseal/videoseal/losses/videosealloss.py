...
class VideosealLoss(nn.Module):
    def __init__(self,
        ...
        #HAYEREMERIPHOTO
        self.detection_loss = torch.nn.CosineEmbeddingLoss()
        self.decoding_loss = torch.nn.CosineEmbeddingLoss()
        #self.detection_loss = torch.nn.MSELoss()
        #self.decoding_loss = torch.nn.MSELoss()
    ...
    def forward(self,
        ...
        if optimizer_idx == 0:  # embedder update
            ...
            if self.detect_weight > 0:
                ...
                target_labels = torch.ones(preds.size(0)).to(preds.device)
