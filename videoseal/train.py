...
Examples:
    ...
    OMP_NUM_THREADS=40 torchrun --nproc_per_node=1 train.py --local_rank 0 \
        ...
        --epochs 1 --iter_per_epoch 1000 --scheduler CosineLRScheduler,lr_min=1e-6,t_initial=601,warmup_lr_init=1e-8,warmup_t=20 --optimizer AdamW,lr=5e-4 \
...
def get_parser():
    ...
    aa("--num_augs", type=int, default=1,
       help="Number of augentations to apply")
    ...
    aa('--saveckpt_freq', default=1, type=int, help='Frequency for saving ckpts')
...
def main(params):
    ...
    uoptim.restart_from_checkpoint(
        os.path.join(params.output_dir, "checkpoint010.pth"),
    ...
    for epoch in range(start_epoch, params.epochs):
        ...
        if epoch >= params.finetune_detector_start:
            ...
            image_detection_loss.percep_weight = 1.0
        ...
        if epoch<2:
            ...
            train_stats = train_one_epoch_type1(wam_ddp, optimizers, epoch_train_loader, epoch_modality, image_detection_loss, epoch, params, tensorboard=tensorboard)
            log_stats = {
                'epoch': epoch, 'modality': epoch_modality, 
                **{f'train_{k}': v for k, v in train_stats.items()}
            }
...
def train_one_epoch_type2(
    ...
    for param in wam.detector.parameters():
    ...
    MESSAGES = torch.load('distinctMAIN.pt', map_location=device)
    ...
    for it, batch_items in enumerate(metric_logger.log_every(train_loader, 10, header)):
        ...
        for acc_it in range(accumulation_steps):
            ...
            rand_indices = torch.randint(0, len(MESSAGES), (batch_size,))
            msg = torch.stack([MESSAGES[i] for i in rand_indices]).to(device)

            outputs = wam(imgs, masks, is_video=is_video,msgs=msg)
            #AnotherFLAG:-
            #outputs["preds"] /= params.temperature
            #print("Preds", outputs["preds"])
...
def train_one_epoch_type1(
    ...
    is_video = (epoch_modality == Modalities.VIDEO)
    torch.autograd.set_detect_anomaly(True)
    for param in wam.embedder.parameters():
        param.requires_grad = False
    wam.train()
...
def eval_one_epoch(
    ...
    for it, batch_items in enumerate(metric_logger.log_every(val_loader, 10, header)):
        ...
        for acc_it in range(accumulation_steps):
            ...
            outputs = wam.embed(imgs, is_video=is_video, lowres_attenuation=params.lowres_attenuation,msgs=msg)
