...
PROMPT_DICT = {
    'cifar10': { 
        "template":"A realistic photo of template*",
        "label_name": ["airplane", "automobile", "bird", "cat", "deer", "dog", "frog", "horse","ship", "truck"],
        },

    'cifar100': {
        "template":"A realistic photo of template*",
        "label_name": ['apple', 'aquarium_fish', 'baby', 'bear', 'beaver', 'bed', 'bee', 'beetle', 'bicycle', 'bottle', 
    ...
    "covid":{
        "template":"template*",
        "label_name": ["A chest X-ray image shows the lungs and ribs, illustrating the manifestation of COVID-19 in a patient.",
                        "A chest X-ray image shows the lungs and ribs, illustrating the manifestation of Viral Pneumonia in a patient.",
                        "A chest X-ray image shows the lungs and ribs, illustrating a normal and healthy condition.",]
        },
    
    "imagenette":{
        "template":"A realistic photo of template*",
        "label_name": [ "tench, Tinca tinca", "English springer, English springer spaniel", 
                        "cassette player", "chain saw, chainsaw",
                        "church, church building", "French horn, horn", 
                        "garbage truck, dustcart", "gas pump, gasoline pump, petrol pump, island dispenser",
                        "golf ball", "parachute, chute"] 
        },
    
    "imagenet1k":{
        "template":"A realistic photo of template*",
        "label_name":list(IMAGENET1K_DICT.values())
        },
    "domainnet":{
        "template":"an image of template*",
        "label_name":['airplane', 'clock', 'axe', 'basketball', 'bicycle', 'bird', 'strawberry', 'flower', 'pizza', 'bracelet']},
        # "label_name":['aircraft carrier', 'airplane', 'alarm clock', 'ambulance', 'angel', 'animal migration', 'ant', 
...
def generate_class_prompts(label_name, data_type):
    ...
    if label_name:
        return [
            PROMPT_DICT[data_type]["template"].replace("template*", label)
            for label in PROMPT_DICT[data_type]["label_name"]
        ] 

def precompute_text_embeddings(class_prompts, tokenizer, text_encoder, unet, batch_size_generation):
    """
    ...
    for class_index in tqdm(range(len(class_prompts))):
        text_inputs = tokenizer(
            [class_prompts[class_index]] * batch_size_generation,
            padding="max_length",
