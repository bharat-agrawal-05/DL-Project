...
class VAE(nn.Module): # This VAE class seems unused in the main flow.
...
def generate_images_WIP(accelerator, class_index, single_class_text_embeddings, uncond_embeddings,
    ...
    # MODIFIED: Save directly to the main path, not in class-specific subdirectories
    image_save_dir_path = config.save_syn_data_path
    os.makedirs(image_save_dir_path, exist_ok=True) # Ensures the main save directory exists

    # unet, sd_vae_decoder, text_encoder (via uncond_embeddings) might be DDP wrapped.
    # Access .module if needed for specific non-DDP operations or attribute access.
    # For standard forward passes, the DDP wrapper handles it.
    # Let's get the unwrapped versions for clarity if direct attributes are needed.
    _unet = unet.module if hasattr(unet, 'module') else unet
    _sd_vae_decoder = sd_vae_decoder.module if hasattr(sd_vae_decoder, 'module') else sd_vae_decoder
    ...
    latents_shape = (config.batch_size_generation, _unet.config.in_channels, 64, 64) # Use _unet here
    ...
    for timestep_idx, timesteps in enumerate(tqdm(timesteps_tensor[:-1], desc=f"Class {class_index} Steps")):
        ...
        with torch.no_grad():
            noise_pred_unet_output = unet(latent_model_input, timesteps, input_embeddings).sample # unet (wrapped) for forward pass
        ...
        if apply_dgm_guidance:
            ...
            with torch.enable_grad():
                ...
                input_latents_for_sd_vae = (pred_original_sample_float / config.sd_vae_scale_factor).to(_sd_vae_decoder.dtype) # Use _sd_vae_decoder.dtype
                image_from_sd = sd_vae_decoder.decode(input_latents_for_sd_vae).sample # sd_vae_decoder (wrapped) for forward pass
        ...
        if config.save_intermediate_images and (timestep_idx + 1) % config.save_intermediate_every_n_steps == 0 :
            with torch.no_grad():
                ...
                input_latents_for_sd_vae_save = (pred_original_sample_to_save / config.sd_vae_scale_factor).to(_sd_vae_decoder.dtype) # Use _sd_vae_decoder.dtype
                image_decoded_save = sd_vae_decoder.decode(input_latents_for_sd_vae_save).sample # sd_vae_decoder (wrapped) for forward
        ...
        with torch.no_grad():
            scheduler_output_step = noise_scheduler.step(final_noise_pred_for_step.to(_unet.dtype), timesteps.cpu(), latents.detach(), generator=generator, return_dict=True) # Use _unet.dtype
            latents = scheduler_output_step.prev_sample.to(weight_dtype)
        if hasattr(unet, 'training') and unet.training: unet.zero_grad(set_to_none=True) # Check if unet is nn.Module before calling zero_grad
    ...
    with torch.no_grad():
        ...
        final_latent_model_input = torch.cat([latents.to(device=accelerator.device, dtype=_unet.dtype)] * 2) # Use _unet.dtype
        final_noise_preds_unet_output = unet(final_latent_model_input, final_loop_timestep, final_input_embeddings).sample # unet (wrapped) for forward
        ...
        scheduler_output_final = noise_scheduler.step(final_model_pred_for_saving.to(_unet.dtype), final_loop_timestep.cpu(), latents.to(_unet.dtype), generator=generator, return_dict=True) # Use _unet.dtype
        ori_latents_final = scheduler_output_final.pred_original_sample
        input_latents_final_decode = (ori_latents_final / config.sd_vae_scale_factor).to(_sd_vae_decoder.dtype) # Use _sd_vae_decoder.dtype
        image_final_decoded = sd_vae_decoder.decode(input_latents_final_decode).sample # sd_vae_decoder (wrapped) for forward
...
class Config:
    def __init__(self):
        self.SEED = 5000
        self.name = "DATASET2"
        self.save_syn_data_path = "./DATASET2"
        self.checkpoints_dir = "./checkpoints"
        self.generate_nums = 20
        self.num_images_to_generate_total = 200
...
def main(config:Config):
    ...
    # load_models_and_tokenizer likely calls accelerator.prepare internally
    # so tokenizer, text_encoder, etc. might already be prepared (wrapped if DDP)
    ...
    # Access .module if the models are wrapped by DDP/accelerate for direct attribute access
    # The variables text_encoder, sd_vae_decoder, unet returned by load_models_and_tokenizer
    # are the ones that might be wrapped.
    _text_encoder = text_encoder.module if hasattr(text_encoder, 'module') else text_encoder
    _sd_vae_decoder = sd_vae_decoder.module if hasattr(sd_vae_decoder, 'module') else sd_vae_decoder
    _unet = unet.module if hasattr(unet, 'module') else unet

    print(f"Loaded SD models. UNet: {_unet.dtype}, SD VAE: {_sd_vae_decoder.dtype}, TextEnc: {_text_encoder.dtype}, Latents: {weight_dtype}")

    # When calling .to(accelerator.device) or .eval(), call it on the original DDP-wrapped object
    # The DDP wrapper will handle propagating this to the underlying module.
    # If load_models_and_tokenizer already puts them on device and in eval mode, these might be redundant.
    # Assuming they are not redundant for now.
    unet.to(accelerator.device).eval()
    text_encoder.to(accelerator.device).eval()
    sd_vae_decoder.to(accelerator.device).eval()

    # For functions that need the unwrapped model (e.g., for config or specific attributes), pass the unwrapped one.
    # For dtype, use the unwrapped model's dtype.
    uncond_embeddings = prepare_uncond_embeddings(tokenizer, _text_encoder, _unet, config.batch_size_generation)
    uncond_embeddings = uncond_embeddings.to(accelerator.device, dtype=_text_encoder.dtype)
    ...
    try:
        ...
        # accelerator.prepare can also handle the videoseal model if it needs to be distributed
        # or put on the correct device. For now, manual .to(device) is fine if it's not DDP.
        videoseal_model_instance = videoseal_model_instance.to(accelerator.device).eval()
    ...
    list_of_class_text_embeddings, _ = precompute_text_embeddings(class_prompts, tokenizer, _text_encoder, _unet, config.batch_size_generation)
    class_text_embeddings = [emb.to(accelerator.device, dtype=_text_encoder.dtype) for emb in list_of_class_text_embeddings]
    ...
    for set_idx in tqdm(range(config.generate_nums), desc="Overall Generation Sets"):
        ...
        for class_idx_loop in range(num_classes_this_set):
            ...
            # Pass the original (potentially wrapped) models from load_models_and_tokenizer
            # to generate_images_WIP. Inside generate_images_WIP, it will unwrap them if needed for attribute access.
